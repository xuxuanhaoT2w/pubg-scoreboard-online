...
export function LobbyPage() {
  ...
  useEffect(() => { void listGlobalPlayers().then(setGlobalPlayers).catch(() => undefined); }, []);
  ...
  const addGlobal = async () => { const name = newGlobalName.trim(); if (!name) return; try { const player = await addGlobalPlayer(name); setGlobalPlayers((prev) => [...prev, player]); setNewGlobalName(''); } catch { setLocalErr('名称已存在或保存失败'); } };
  ...
  return <div className="mx-auto flex min-h-[100dvh] max-w-md flex-col items-center justify-center px-6 py-10">
    ...
    <div className="mt-8 w-full space-y-3">
      ...
      {mode === 'global' && <div className="tac-card space-y-3 p-5"><h2 className="font-display text-lg font-bold">全局人员库</h2><p className="text-xs text-ink-muted">可在新建房间时直接选取；同名人员不会重复保存。</p><div className="flex gap-2"><input className="tac-input h-10 flex-1" value={newGlobalName} maxLength={12} placeholder="新增全局人员" onChange={(e) => setNewGlobalName(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') void addGlobal(); }} /><button type="button" className="tac-btn tac-btn-primary h-10 px-3" onClick={() => void addGlobal()}><Plus size={16} /></button></div>{localErr && <p className="text-sm text-loss">{localErr}</p>}<div className="space-y-2">{globalPlayers.map((player) => <div key={player.id} className="flex items-center rounded-lg border border-line bg-panel-2 px-3 py-2"><span className="flex-1 text-sm font-semibold">{player.name}</span><button type="button" className="text-loss" aria-label="删除全局人员" onClick={() => void removeGlobal(player.id)}><Trash2 size={15} /></button></div>)}</div><button type="button" className="tac-btn h-11 w-full" onClick={() => setMode('menu')}>返回</button></div>}
